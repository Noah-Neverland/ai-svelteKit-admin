...
#### 1.2.1 状态管理
- **Svelte Stores**：Svelte 内置状态管理，轻量且高效
- **使用场景**：用户信息、主题配置、语言设置、菜单状态等全局状态
...
- **主题 Store**：`theme.ts` store 管理当前主题色
- **主题文件**：`src/lib/config/theme.config.ts` 定义 shadcn-svelte 主题色
- **动态切换**：通过修改 `:root` CSS 变量实现主题切换
- **持久化**：主题选择保存到 localStorage

#### 4.4.2 黑白模式切换
**技术方案**：
- **Tailwind 暗色模式**：使用 `dark:` 前缀
- **模式 Store**：`theme.ts` store 管理当前模式
- **动态切换**：通过修改 `<html>` 的 `class` 或 `data-theme` 属性
...
#### 5.1.1 认证 Store（auth.ts）
**功能**：
- 管理登录状态
- 存储 token
- 提供登录、登出方法

#### 5.1.2 用户 Store（user.ts）
**功能**：
- 存储用户信息
- 提供更新用户信息方法

#### 5.1.3 菜单 Store（menu.ts）
**功能**：
- 存储菜单数据
- 管理菜单展开/折叠状态
- 提供菜单搜索功能

#### 5.1.4 主题 Store（theme.ts）
**功能**：
- 管理主题色
- 管理黑白模式
- 持久化主题设置

#### 5.1.5 国际化 Store（i18n.ts）
**功能**：
- 管理当前语言
- 提供语言切换方法
- 持久化语言设置
